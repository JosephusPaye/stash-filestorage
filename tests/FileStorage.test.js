...
test('FileStorage.size() retrieves the number of items in storage', async () => {
  await useTempFile(async (filePath) => {
    ...
    // Recreate storage to read from file
...
test('FileStorage.has() determines whether an item is stored by its key', async () => {
  await useTempFile(async (filePath) => {
    let storage = new FileStorage({
      filePath,
      bufferInMemory: true,
...
test('FileStorage.get() retrieves the value of an item by its key', async () => {
  await useTempFile(async (filePath) => {
    let storage = new FileStorage({
      filePath,
      bufferInMemory: true,
...
test('FileStorage.set() adds an item to storage, overwriting existing keys', async () => {
  await useTempFile(async (filePath) => {
    let storage = new FileStorage({
      filePath,
      bufferInMemory: true,
...
test('FileStorage.delete() removes an item from storage', async () => {
  await useTempFile(async (filePath) => {
    let storage = new FileStorage({
      filePath,
      bufferInMemory: true,
...
test('FileStorage.clearMatching() removes items that match a given criteria', async () => {
  await useTempFile(async (filePath) => {
    let storage = new FileStorage({
      filePath,
      bufferInMemory: true,
    ...
    assert.is(
      await storage.has('k2'),
      false,
      'first matching item was cleared'
    );
    assert.is(
      await storage.has('k4'),
      false,
      'second matching item was cleared'
    );

    assert.is(await storage.has('k1'), true, 'first non-matching item remains');
    assert.is(
      await storage.has('k3'),
      true,
      'second non-matching item remains'
    );
  });
});

test('FileStorage.clear() removes all items in storage', async () => {
  await useTempFile(async (filePath) => {
    let storage = new FileStorage({
      filePath,
      bufferInMemory: true,
